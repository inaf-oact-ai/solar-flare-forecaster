...
def parse_args():
    ...
    p.add_argument("--label_var_name", type=str, default="label", help="Name of label variable in input file. Common options: {'label','flare_type'}")
    return p.parse_args()
...
def group_by_ar(records: List[dict], label: str, label_var_name: str) -> Dict[int, List[int]]:
    """
    Returns dict: ar -> list of indices (positions in 'records') for entries with given label in that AR.
    """
    groups = defaultdict(list)
    for idx, rec in enumerate(records):
        if rec.get(label_var_name) == label:
            ar = rec.get("ar")
...
def main():
    ...
    # Split by label
    idx_none = [i for i, r in enumerate(records) if r.get(args.label_var_name) == LABEL_NONE]
    idx_c    = [i for i, r in enumerate(records) if r.get(args.label_var_name) == LABEL_C]
    idx_other = [i for i, r in enumerate(records) if r.get(args.label_var_name) not in (LABEL_NONE, LABEL_C)]
    ...
    if target_none != n_none:
        none_groups = group_by_ar(records, LABEL_NONE, args.label_var_name)
        keep_none_plan, actually_kept_none = compute_removal_plan(none_groups, target_none, rng)
    else:
        actually_kept_none = n_none

    if target_c != n_c:
        c_groups = group_by_ar(records, LABEL_C, args.label_var_name)
    ...
    keep_none_indices, remove_none_indices = _apply_keep_plan(records, LABEL_NONE, keep_none_plan, rng, args.label_var_name)
    keep_c_indices,    remove_c_indices    = _apply_keep_plan(records, LABEL_C,    keep_c_plan,    rng, args.label_var_name)
    ...
    # Summaries
    out_none = sum(1 for r in out_records if r.get(args.label_var_name) == LABEL_NONE)
    out_c    = sum(1 for r in out_records if r.get(args.label_var_name) == LABEL_C)
    out_total = len(out_records)

    # Per-AR summaries
    before_none_counts = _counts_from_indices(records, idx_none)
    after_none_counts  = _counts_from_records(out_records, LABEL_NONE, args.label_var_name)

    before_c_counts = _counts_from_indices(records, idx_c)
    after_c_counts  = _counts_from_records(out_records, LABEL_C, args.label_var_name)
...
def _apply_keep_plan(records: List[dict], label: str, keep_plan: Dict[int, int], rng: random.Random, label_var_name: str):
    ...
    if not keep_plan:
        ...
        indices = [i for i, r in enumerate(records) if r.get(label_var_name) == label]
        return indices, []

    groups = group_by_ar(records, label, label_var_name)
    keep_indices = []
    remove_indices = []
...
def _counts_from_records(records_list, label, label_var_name):
    counts = defaultdict(int)
    for r in records_list:
        if r.get(label_var_name) == label:
            counts[r.get("ar")] += 1
    return counts
