...
def prepare_bg_day(bg: pd.DataFrame) -> pd.DataFrame:
    """Precompute per-day background mapping once."""
    bg_day = (
        bg[["date", "background"]]
...
def slice_ratio_on_window_v2(start: pd.Timestamp,
    ...
    frac_valid = seg["science"].notna().mean()
    if np.isnan(frac_valid) or frac_valid < min_coverage:
        raise RuntimeError(f"Coverage {frac_valid:.3f} "
                           f"below threshold {min_coverage:.3f}")

    seg["science"] = seg["science"].interpolate(limit_direction="both")

    # join background per-day using precomputed bg_day
    day_df = pd.DataFrame(index=grid)
    day_df["date"] = day_df.index.floor("D")
    day_df = day_df.join(bg_day, on="date")
    ...
    ratio = (seg["science"].values.astype("float64") /
             day_df["background"].values.astype("float64")).astype("float32")
    return grid, ratio
...
def compute_ts_only_label(t_end: pd.Timestamp,
    ...
    # events_df is indexed by t_occ now
    # Use (label_start, label_end] ≈ [label_start_plus_epsilon, label_end]
    label_start_inc = label_start + pd.Timedelta(seconds=1)
    occ = events_df.loc[label_start_inc:label_end]
...
#        "video_label","ts_label","ts_id","equal_flag"
    ...
    events_df["t_occ"] = pd.to_datetime(events_df["t_occ"], utc=True)
    events_df = events_df.sort_values("t_occ").set_index("t_occ")

    print("Reading science & bkg data ...")
    sci, bg, sat_name = load_science_and_background(args)
    
    print("Preparing bkg day data ...")
    bg_day = prepare_bg_day(bg)
    ...
    for item in meta["data"]:
        ...
        try:
            ...
            grid, ratio = slice_ratio_on_window_v2(t_start, t_end, sci, bg_day, args.min_coverage)
